...
export default function Landing({ onLaunch }) {
  ...
  return (
    <div className="relative h-screen w-screen overflow-y-auto no-scrollbar bg-bg">
      {/* Subtle aurora — not decorative, ambient only */}
      ...
      <div className="relative z-10 flex flex-col items-center justify-center min-h-screen px-6 py-12">
        ...
        >
          ...
          <span className="text-xs font-mono uppercase tracking-wider">
            {loading ? 'Connecting...' : isLive ? 'Live' : 'Error'}
          </span>
        </motion.div>
        ...
        {/* Real measured state — no fake numbers */}
        <motion.div
          initial={{ opacity: 0, y: 20 }}
          animate={{ opacity: 1, y: 0 }}
        ...
        >
          <p className="text-xs text-secondary/50 text-center font-mono">Index. Query. Retrieve. Verify. Revoke.</p>
          <p className="text-xs text-secondary/50 text-center font-mono">Built for Hugging Face. Measured in receipts.</p>
        </motion.div>
        ...
        <a
          ...
          className="flex items-center gap-1 text-[10px] text-secondary/40 hover:text-secondary transition-colors"
        >
